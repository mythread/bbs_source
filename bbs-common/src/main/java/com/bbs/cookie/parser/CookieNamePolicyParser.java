...
/**
 * 类CookieNamePolicyParser.java的实现描述：TODO 类实现描述
 * 
 * @author jie.xu 2014年11月23日 下午8:52:48
 */
public class CookieNamePolicyParser {
	private static final Logger logger = LoggerFactory
			.getLogger(CookieNamePolicyParser.class);
	...
	static {
		...
		for (Field field : allCookieGroups) {
			CookieNamePolicy cookieNamePolicy = field
					.getAnnotation(CookieNamePolicy.class);
			if (cookieNamePolicy == null) {
				throw new IllegalArgumentException("有一个CookieName:"
						+ field.getName() + "没有配置CookieNamePolicy");
			}
			// 初始化当前的CookieNameConfig
			CookieNameEnum cookieNameEnum = CookieNameEnum.valueOf(field
					.getName());
			CookieNameConfig CookieNameConfig = new CookieNameConfig(
					cookieNameEnum, cookieNamePolicy);
		...
		for (Field field : allCookieKeys) {
			CookieKeyPolicy cookiePolicy = field
					.getAnnotation(CookieKeyPolicy.class);
			if (cookiePolicy == null) {
				throw new IllegalArgumentException("CookieKey:["
						+ field.getName() + "]没有配置CookiePolicy");
			}
			...
			CookieNameConfig cookieNameConfig = COOKIE_NAME_CONFIG_MAP
					.get(cookieName);
			cookieNameConfig.appendKey(cookieKey);

		}
		// unmodified
		COOKIE_NAME_CONFIG_MAP = Collections
				.unmodifiableMap(COOKIE_NAME_CONFIG_MAP);
		if (logger.isDebugEnabled()) {
